...
class TankZombie:
    ...
    def __init__(self, x: float, y: float):
        ...
        """
        # Configuration
        self.config = tank_zombie_config
        ...
        logger.debug(
            f"TankZombie spawned at ({int(x)}, {int(y)}) - HP: {self.health}/{self.max_health}"
        )
    ...
    def take_damage(self, amount: int) -> bool:
        ...
        """
        self.health -= amount
        logger.debug(f"TankZombie took {amount} damage, health: {self.health}/{self.max_health}")

        if self.health <= 0:
            logger.debug(f"TankZombie died at ({int(self.x)}, {int(self.y)})")
            return False
        return True
    ...
    def update(self, delta_time: float, player_x: float, player_y: float) -> None:
        ...
        if distance > 0:
            ...
            # Move toward player (slower than normal zombie!)
            ...
            # Tank zombies rotate slower (360°/sec = 1 rotation/sec)
